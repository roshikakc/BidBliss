...
$query = "SELECT * FROM `products` WHERE `product_category` = 'decorative_arts'";
$result = mysqli_query($conn, $query) or die('Query failed');

// Store products in an array
$decorativeArtsProducts = [];
if (mysqli_num_rows($result) > 0) {
  while ($row = mysqli_fetch_assoc($result)) {
    $decorativessProducts[] = $row;
  }
}
?>
...
<body>
  ...
  <section id="upcomming" class="section-m1">
    ...
    <div class="wrapper">
      <?php
      foreach ($decorativeArtsProducts as $product) {
